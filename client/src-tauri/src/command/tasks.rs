...
pub async fn list_tasks(
    ...
    let mut client = tasks_client(&*state, &address, use_tls)
        .await
    ...
    let tasks: Vec<dto::TaskDto> = resp
        .into_inner()
        .tasks
        .into_iter()
        .map(convert::task_from_proto)
        .collect();
...
pub async fn create_task(
    ...
    let workflow = Some(crate::command::tasks_proto::Workflow {
        steps: input
            ...
            .steps
            .into_iter()
            .map(|s| crate::command::tasks_proto::WorkflowStep {
                r#type: s.r#type,
                tool: s.tool,
            })
            .collect(),
...
pub async fn delete_task(
    ...
    let mut client = tasks_client(&*state, &address, use_tls)
        ...
        .context("Failed to connect to server")?;
    let req =
        tonic::Request::new(crate::command::tasks_proto::DeleteTaskRequest { id: id.clone() });
    client
        .delete_task(req)
        .await
...
pub async fn restart_task(
    ...
    let mut client = tasks_client(&*state, &address, use_tls)
        .await
        .context("Failed to connect to server")?;
    let req = tonic::Request::new(crate::command::tasks_proto::RestartTaskRequest {
        id: id.clone(),
        clear_logs,
        start_now,
    });
    ...
    tauri::async_runtime::spawn(async move {
        ...
        match client.stream_task_events(req).await {
            ...
            Err(e) => {
                let _ = window.emit(
                    &event_topic,
                    crate::utils::dto::TaskEventDto::Error(crate::utils::dto::ErrorDto {
                        message: e.to_string(),
                    }),
                );
            }
