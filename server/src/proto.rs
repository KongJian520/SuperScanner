use crate::proto::server_status::ServerInfoService;
use crate::proto::status_proto::server_info_server;
use crate::proto::server_tasks::TasksService;
use crate::proto::tasks_proto::tasks_server;
use crate::storage::TaskStore;
use crate::utils::ROOT_DIR;
use crate::core::command::CommandRegistry;
use std::sync::Arc;

pub mod server_tasks;
pub mod server_status;

pub mod tasks_proto {
    tonic::include_proto!("tasks.v1");
}
pub mod status_proto {
    tonic::include_proto!("status.v1");
}




// Construct a TasksServer given a ready TaskStore (async init should happen in main)
pub fn tasks_svc_with_store(store: Arc<dyn TaskStore>, registry: CommandRegistry) -> tasks_server::TasksServer<TasksService> {
    tasks_server::TasksServer::new(TasksService::new_with_store(ROOT_DIR.join("tasks"), store, registry))
}

pub fn server_info_svc() -> server_info_server::ServerInfoServer<ServerInfoService> {
    server_info_server::ServerInfoServer::new(ServerInfoService::new())
}
